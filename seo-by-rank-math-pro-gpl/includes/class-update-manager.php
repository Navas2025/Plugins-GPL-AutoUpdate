...
class Rank_Math_Pro_GPL_Update_Manager {
    ...
    const PLUGIN_SLUG = 'seo-by-rank-math-pro-gpl';
    const PLUGIN_FILE = 'seo-by-rank-math-pro-gpl/rank-math-pro-gpl.php';
    
    public function __construct() {
        error_log('✅ [Rank_Math_Pro_GPL_Update_Manager] inicializado');
    ...
    public function check_for_plugin_update($transient) {
        error_log('=== [RANK MATH PRO] GPL - CHECK FOR UPDATE ===');
        ...
        if ($status === 'active' && !empty($expiry_date)) {
            ...
            if ($current_timestamp >= $expiry_timestamp) {
                ...
                error_log('⚠️ Licencia CADUCADA');
            }
        }

        if (!$current_version || empty($api_key) || $status !== 'active') {
            error_log('❌ SALIENDO: version=' . ($current_version ?: 'false') . ' | key=' . ($api_key ? 'OK' : 'NO') . ' | status=' . $status);
        ...
        $url = add_query_arg($args, $server_url . 'get-plugins.php');
        error_log('🌐 URL: ' . $url);
